...
export default function Quiz() {
  ...
  const [examType, setExamType] = useState<string>('CC301')

  useEffect(() => {
    const data = examType === 'CC301' ? questionsData : arcQuestionsData
    const shuffledQuestions = shuffleArray(data as Question[])
      ...
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center p-4 transition-colors duration-200">
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            ...
            />
            <Label htmlFor="examType" className="text-lg mt-4">Select Exam Type:</Label>
            <select
              id="examType"
              value={examType}
              onChange={(e) => setExamType(e.target.value)}
              className="mt-2 text-lg w-full p-2 border rounded"
            >
              <option value="CC301">CC301</option>
              <option value="ARC301">ARC301</option>
            </select>
          </CardContent>
